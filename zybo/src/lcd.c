...
static XGpio
    gpioTftControl; // Provides the RD, WR and CD pins for the LCD controller.
static XGpio
    gpioTftDataBus; // Provides an 8-bit data bus for the LCD controller.
static bool initFlag =
    false; // Make sure that body of init routine only gets invoked once.
...
void LCD_init() {
  ...
  status = XGpio_Initialize(&gpioTftControl,
                            LCD_CONTROL_DEVICE_ID); // Xilinx GPIO init call.
  if (status != XST_SUCCESS) {
    printf("XGPIO_Initialize (TFT) failed\n.");
  }
  status = XGpio_Initialize(&gpioTftDataBus,
                            LCD_DATA_BUS_DEVICE_ID); // Xilinx GPIO init call.
  if (status != XST_SUCCESS) {
    printf("XGPIO_Initialize (TFT Data Bus) failed\n.");
  }
  // Set the direction for all signals to be outputs (0 = output, 1 = input).
  XGpio_SetDataDirection(&gpioTftControl, 1,
                         0); // Control bits are always outputs.
  XGpio_SetDataDirection(&gpioTftDataBus, 1,
                         0); // Set up data-bus direction as output (write).
  mio_init(true);
...
// Sets the logic value on the command/data pin for the LCD controller to
// command mode.
void LCD_setCommandMode() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue & ~LCD_DCX_BIT_MASK); // Clears the DCX bit.
}

// Sets the logic value on the command/data pin for the LCD controller to data
// mode.
void LCD_setDataMode() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue | LCD_DCX_BIT_MASK); // Sets the DCX bit.
}

// Set the logic value on the LCD RD pin for read operations for the LCD data
// bus.
void LCD_assertRd() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue & ~LCD_RD_BIT_MASK); // Asserts RD
}

// Set the logic value on the LCD RD pin to disable read operations on the LCD
// data bus.
void LCD_negateRd() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue | LCD_RD_BIT_MASK); // Negates RD
}

// Set the logic value on the LCD WR pin to enable write operations on the LCD
// data bus.
void LCD_assertWr() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue & ~LCD_WR_BIT_MASK); // Asserts WR
}

// Set the logic value on the LCD WR pin to disable write operations on the LCD
// data bus.
void LCD_negateWr() {
  uint32_t regValue = XGpio_DiscreteRead(&gpioTftControl, 1);
  XGpio_DiscreteWrite(&gpioTftControl, 1,
                      regValue | LCD_WR_BIT_MASK); // Negates WR
}

// These are utility functions

// Most of these delay functions are disabled as they were not found to be
// necessary during testing. They are kept around to act as place-holders in the
// original source code from the Adafruit site.
...
void LCD_setReadDataDirection() {
  ...
  XGpio_SetDataDirection(&gpioTftDataBus, 1,
                         0xFF); // LCD data bus is input from ZYNQ perspective.
}

// Set the GPIO pins on the LCD data bus for write operations.
void LCD_setWriteDataDirection() {
  LCD_negateRd(); // Take care to make sure that two drivers are not enabled at
                  // the same time.
  XGpio_SetDataDirection(&gpioTftDataBus, 1,
                         0); // LCD data bus is output from ZYNQ perspective.
}
...
void LCD_writeData(uint8_t value) {
  XGpio_DiscreteWrite(&gpioTftDataBus, 1,
                      value); // Perform the write using Xilinx GPIO call.
}

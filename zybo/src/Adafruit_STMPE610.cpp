...
void Adafruit_STMPE610::readData(int16_t *x, int16_t *y, uint8_t *z) {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    data[i] = readRegister8(
        0xD7); // SPI.transfer(0x00); 0xD7 is non-auto-increment mode.
...
// BLH: I wrote this and it does not seem to work correctly as of yet. (OK, kind
// of works now).
...
uint8_t Adafruit_STMPE610::spiIn() {
  ...
  spi_setBitOrder(
      SPI_MSBFIRST); // wasn't included in the original code. added by BLH.8
...
void Adafruit_STMPE610::spiOut(uint8_t x) {
  ...
  spi_setClockDivider(84);
  spi_setBitOrder(
      SPI_MSBFIRST); // wasn't included in the original code. added by BLH.
                     //    SPI.setDataMode(m_spiMode);
  spi_setTransmissionMode(m_spiMode);
  //    SPI.transfer(x);
  spi_transfer(x);
...
uint8_t Adafruit_STMPE610::readRegister8(uint8_t reg) {
  ...
  spi_setTouchScreenControllerSlaveSelect(); // Assert the slave select for the
...
// BLH
// This only works properly for tft.setRotation(1). This is portrait mode with
// the origin located at the lower-left-hand corner of the LCD screen (this is
// with the screen mounted so that the parallel interface is located on the
// right-hand-side of the LCD display. The origin of the LCD display is located
// upper left-hand-side corner. The origin of the touch-screen is located lower
// left-hand-side corner. This routine maps and scales the touch-screen
// coordinates to the LCD coordinate space.
...
//  // y runs in the opposite direction, so it needs to be flipped and then
//  scaled. float yFloat = TOUCH_SCREEN_MAX_Y - ((float) y);

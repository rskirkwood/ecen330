...
// The MIO system is the PS GPIO that communicates with the MIO pins on the ZYBO
// board. MIO pins: 13, 10, 11, 12, 0, 9, 14, 15 are connected to JF pins: 1, 2,
// 3, 4, 7, 8, 9, 10. MIO pin 7 is connected to LED LD4 (located to the right of
// JF). BTN5 is connected to MIO51 BTN4 is connected to MIO50

static bool mioInitFlag =
    false; // Ensures the init routine will be invoked only once.
...
// The boolean parameter allows you to enable/disable the printing of error
// messages.
int mio_init(bool printFailedStatusFlag) {
  if (!mioInitFlag) { // Only do this once.
    ConfigPtr = XGpioPs_LookupConfig(
        XPAR_XGPIOPS_0_DEVICE_ID); // Lookup the config info for the MIO GPIO
                                   // device.
    int status = XGpioPs_CfgInitialize(&mioGpio, ConfigPtr,
                                       ConfigPtr->BaseAddr); // Initialize it..
    if (status != XST_SUCCESS) {   // Check for errors.
      ...
      return XST_FAILURE; // Return the value of the failure code to aid
                          // debugging.
    }
  }
...
// Provides a more readable function name and also checks to see if the init
// routine was called.
...
// Does the write as a single bank write. Checks to see that the init routine
// was called.
...
void mio_setPinAsOutput(u8 mioPinNo) {
  if (!mioInitFlag) {
    ...
    XGpioPs_SetDirectionPin(
        &mioGpio, mioPinNo,
        MIO_OUTPUT_PIN_CONFIGURATION); // This configures the output direction.
    XGpioPs_SetOutputEnablePin(&mioGpio, mioPinNo,
                               1); // This enables the output.
  }
}

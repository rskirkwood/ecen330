...
// The bit-order is specified using #defines from the spi.h file: SPI_LSBFIRST
// or SPI_MSBFIRST.
void spi_setBitOrder(uint8_t bitOrder) {
  ...
  case SPI_LSBFIRST:
    ...
    spi_writeRegister(SPI_CNTRL_REG_OFFSET,
                      regValue | SPI_CNTRL_REG_LSB_FIRST_MASK);
    break;
  case SPI_MSBFIRST:
    regValue = spi_readRegister(SPI_CNTRL_REG_OFFSET);
    spi_writeRegister(SPI_CNTRL_REG_OFFSET,
...
// Does nothing but is provided for compatability with the adafruit
// touch-controller code.
void spi_setClockDivider(uint8_t divider) {
  // Does nothing for this core. Divider is set in hardware when hardware is
...
// This actually does the bit transfer. It will block until the transfer is
// complete. It uses the transfer sequence (mostly) outlined in the Xilinx SPI
// documentation (see above).
uint8_t spi_transfer(uint8_t val) {
  // Assume that bit-order and the mode are configured elsewhere (spiread,
  // spiwrite). Clear all of the slave selects.
  ...
  // (2) manual-slave-select-assertion-enable to 1 = slave select register is
  // asserted at pin when configured
  //     as master and the device is enabled (SPI-system-enable = 1).
  // (3) sets the Master-mode bit = 1.
  // (4) sets the SPI-system-enable bit = 1 (enables the system, makes outputs
  // valid.
  ...
  spi_setControlRegisterBits(SPI_CNTRL_REG_MASTER_TRANSACTION_INHIBIT_MASK |
                             ...
                             SPI_CNTRL_MASTER_MASK | SPI_CNTRL_SPE_MASK);
  ...
  //  spi_setTftSlaveSelect(); // digitalWrite(_cs, LOW);  // Comment out for
  //  now. 6/16/2014.
  ...
  //  }
  //  utils_msDelay(1);
  // Blocking call to wait until the RX FIFO is not empty (waiting for the
...
// The slave-select bits determine which of slaves attached to the SPI
// controller are active. Should use only the specific slave-select functions
// shown below.
...
// Software reset of the SPI controller. Needs to be invoked prior to
// programming the SPI controller for consistent operation.
void spi_softwareReset() {
  spi_writeRegister(SPI_RESET_REG_OFFSET, SPI_RESET_REG_MASK);
}

// Blocking call to wait until the TX register is empty (the transmission has
// completed).
void spi_waitUntilTxRegisterIsEmpty() {
  while (
      !(spi_readRegister(SPI_STATUS_REG_OFFET) & SPI_STATUS_REG_TX_EMPTY_MASK))
    ;
}

...
u64 globalTimer_getTimerValue(void) {
  u32 upperTimerCounterRegister =
      globalTimer_readRegister(GLOBAL_TIMER_UPPER_COUNTER_REGISTER);
  //  printf("globalTimer_getTimerValue (upper): %lx\n",
  //  upperTimerCounterRegister);
  u32 lowerTimerCounterRegister =
      globalTimer_readRegister(GLOBAL_TIMER_LOWER_COUNTER_REGISTER);
  //  printf("globalTimer_getTimerValue (lower): %lx\n",
  //  lowerTimerCounterRegister);
  // Read the upper register again to check for roll-over. If the upper register
  // has changed, reread the lower value to keep things coherent.
  u32 upperTimerCounterRegister2 =
      globalTimer_readRegister(GLOBAL_TIMER_UPPER_COUNTER_REGISTER);
  ...
  u64 timerValue =
      upperTimerCounterRegister; // Assign the upper 32 bits into lower 32 bits.
...
void globalTimer_setControlRegisterBit(u8 bitNumber) {
  // Read the control register first so you can OR specific bits and leave
  // others untouched.
  u32 timerControlRegister =
      globalTimer_readRegister(GLOBAL_TIMER_CONTROL_REGISTER);
  timerControlRegister |= (0x1 << bitNumber);
  globalTimer_writeRegister(GLOBAL_TIMER_CONTROL_REGISTER,
...
void globalTimer_clearControlRegisterBit(u8 bitNumber) {
  ...
  u32 timerControlRegister =
      globalTimer_readRegister(GLOBAL_TIMER_CONTROL_REGISTER);
  timerControlRegister &=
      ~(0x1 << bitNumber); // Clear bit by shifting bit, invert and AND.
  globalTimer_writeRegister(GLOBAL_TIMER_CONTROL_REGISTER,
                            timerControlRegister);
}
...
void globalTimer_startTimer(bool printStatusFlag) {
  if (globalTimer_readControlRegisterBit(
          GLOBAL_TIMER_TIMER_ENABLE_BIT_POSITION) &&
      printStatusFlag) {
...
u32 globalTimer_test(bool printStatusFlag) {
  ...
  // Keep track of whether the global timer was running when you started the
  // test so you can restore it.
  u32 timerRunningFlag = globalTimer_readControlRegisterBit(
      GLOBAL_TIMER_TIMER_ENABLE_BIT_POSITION);
  globalTimer_startTimer(
      false); // false prevents the printing of a status message.
  ...
  if (timerValue1 == timerValue2) {
    if (printStatusFlag)
      printf("globalTimer_test: Error, global timer does not seem to be "
             "running.\n");
    error = 1;
  }
  // Now, try stopping the timer and read two consecutive values to see if the
  // counter is really stopped.
  ...
  if (timerRunningFlag) {
    globalTimer_startTimer(
        false); // Was already running, just start it back up.
  } else {
...
//// A duration timer consists of a start time and and end time. The duration is
/// the difference.
// typedef struct {
//  bool timerStarted;
...
// static globalTimer_duration_t
// durationTimers[GLOBAL_TIMER_DURATION_TIMER_COUNT];
//
//// This package provides up to GLOBAL_TIMER_DURATION_TIMER_COUNT duration
/// timers. / Start the duration timer.
// u32 globalTimer_startDurationTimer(u8 timerNumber) {
//  globalTimer_startTimer(false);  // Start the timer if not already running.
//  false = no status message. durationTimers[timerNumber].timerRunning = true;
...
//    printf("globalTimer_stopDurationTimer(): Error - duration timer %d is not
//    running.\n", timerNumber); error = true;
//  } else {
//    durationTimers[timerNumber].timerRunning = false;                    //
//    Note that the timer is no longer running.
//    durationTimers[timerNumber].stopTime = globalTimer_getTimerValue();  //
//    Get the timer end time.
//  }
//  return error;
//}
//
//// Get the duration of the timer. The timer must be stopped to use this
/// function.
// u16 globalTimer_getDurationTimerValue(u8 timerNumber, float *seconds) {
//  u16 error = 0;
//  if (durationTimers[timerNumber].timerRunning) {
//    printf("globalTimer_getDurationTimerValue(): duration timer %d is still
//    running.\n", timerNumber); error = 1;
//  } else if (!durationTimers[timerNumber].timerStarted){
//    printf("globalTimer_getDurationTimerValue(): duration timer %d was never
//    started.\n", timerNumber);
//	error = 1;
//  } else {
//	printf("start value: %llx\n", durationTimers[timerNumber].startTime);
//	printf("stop value: %llx\n", durationTimers[timerNumber].stopTime);
//    *seconds = (float) (durationTimers[timerNumber].stopTime -
//    durationTimers[timerNumber].startTime) 		/ (float)
//    GLOBAL_TIMER_TICKS_PER_SECOND;
//  }
//  return error;
//}
//

...
void circularBuffer_init(circularBuffer_t *cb) {
  ...
  cb->data =
      (uint32_t *)malloc((CIRCULAR_BUFFER_INDEX_MASK + 1) * sizeof(uint32_t));
}

// Just resets the index pointers to start at the zero position, and resets the
// overflow flag. The buffer becomes empty after reset(). Buffer must be reset
// prior to writing anytime it has been read.
...
// writeIndex always points to the location to be written and then is
// incremented.
void circularBuffer_addData(circularBuffer_t *cb, uint32_t datum) {
  ...
  if (cb->writeIndex == 0) { // You know that you are starting the second pass
    cb->firstPassFlag =
        false; // but don't know if you will wrap-around until next write.
  }
}
...
// Reads a data item at a given index. The index is relative, an index of 0 just
// means the first element (oldest) of the circular buffer. Index is always
// masked so it will always be in range. Does not check the index to see if it
// is accessing stored data (for speed). User is responsible to access data in
// range.

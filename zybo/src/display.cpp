...
// Just define these values here. They won't change in practice and I want to
// avoid too much tangling between the LCD control code and the touch-controller
// code. Otherwise, it would make sense to always get these values from the LCD
// controller. These #defines are not shared outside of this .cpp file and so
// don't need to follow the naming convention.
...
#define TOUCH_SCREEN_MAX_X                                                     \
  3950.0 // This is where the touch-screen maxes out in X.
#define TOUCH_SCREEN_MAX_Y 4095.0

static bool initFlag = false; // Only allow init to be called once.
static Adafruit_TFTLCD lcdDisplay =
    Adafruit_TFTLCD(); // Handle to the LCD display.
static Adafruit_STMPE610 touchController = Adafruit_STMPE610();

// This function is provided because utils_msDelay() must have C linkage
// so it can link with the .c files in student labs.
// This function is only to be used for code in display.cpp
#define US_PER_MS 1000
void utils_msDelayDisplayInternal(long msDelay) { usleep(msDelay * US_PER_MS); }
...
// These are functions related to display. Functionality comes from
// Adafruit_GFX.
...
void display_fillScreen(uint16_t color) { lcdDisplay.fillScreen(color); }

void display_invertDisplay(bool i) { lcdDisplay.invertDisplay(i); }
...
void display_setCursor(int16_t x, int16_t y) { lcdDisplay.setCursor(x, y); }

void display_setTextColor(uint16_t c) { lcdDisplay.setTextColor(c); }

void display_setTextColorBg(uint16_t c, uint16_t bg) {
  lcdDisplay.setTextColor(c, bg);
}

void display_setTextSize(uint8_t s) { lcdDisplay.setTextSize(s); }

void display_setTextWrap(bool w) { lcdDisplay.setTextWrap(w); }

void display_setRotation(uint8_t r) { lcdDisplay.setRotation(r); }

int16_t display_height() { return lcdDisplay.height(); }

int16_t display_width() { return lcdDisplay.width(); }
...
size_t display_println(const char str[]) { return lcdDisplay.println(str); }
size_t display_printlnChar(char c) { return lcdDisplay.println(c); }
size_t display_printlnDecimalInt(int num) {
  return lcdDisplay.println(num, 10);
}
}
...
size_t display_println(void) { return lcdDisplay.println(); }

extern "C" {

size_t display_print(const char str[]) { return lcdDisplay.print(str); }
size_t display_printChar(char c) { return lcdDisplay.print(c); }
size_t display_printDecimalInt(int num) { return lcdDisplay.print(num, 10); }
}
...
size_t display_print(int num, int base) { return lcdDisplay.print(num, base); }

size_t display_print(unsigned int num, int base) {
  return lcdDisplay.print(num, base);
}

size_t display_print(long num, int base) { return lcdDisplay.print(num, base); }
...
bool display_isTouched(void) { return touchController.touched(); }
...
void display_mapToLcdCoordinates(int16_t *x, int16_t *y) {
  ...
  // and offset the maximum value so it matches the max LCD coordinate at the
  // far right of the LCD.
...
void display_clearOldTouchData() { touchController.clearOldTouchData(); }
...
unsigned long display_testLines(uint16_t color) {
  int x1, y1, x2, y2, w = display_width(), h = display_height();
...
unsigned long display_testFastLines(uint16_t color1, uint16_t color2) {
  ...
  display_fillScreen(DISPLAY_BLACK);
  for (y = 0; y < h; y += 5)
    display_drawFastHLine(0, y, w, color1);
  for (x = 0; x < w; x += 5)
    display_drawFastVLine(x, 0, h, color2);
  return 0;
}

unsigned long display_testRects(uint16_t color) {
  int n, i, i2, cx = display_width() / 2, cy = display_height() / 2;
...
unsigned long display_testFilledRects(uint16_t color1, uint16_t color2) {
  int n, i, i2, cx = display_width() / 2 - 1, cy = display_height() / 2 - 1;
...
unsigned long display_testCircles(uint8_t radius, uint16_t color) {
  int x, y, r2 = radius * 2, w = display_width() + radius,
...
unsigned long display_testTriangles() {
  int n, i, cx = display_width() / 2 - 1, cy = display_height() / 2 - 1;

  display_fillScreen(DISPLAY_BLACK);
  n = min(cx, cy);
  for (i = 0; i < n; i += 5) {
    display_drawTriangle(cx, cy - i,     // peak
...
unsigned long display_testFilledTriangles() {
  int i, cx = display_width() / 2 - 1, cy = display_height() / 2 - 1;
...
unsigned long display_testRoundRects() {
  int w, i, i2, cx = display_width() / 2 - 1, cy = display_height() / 2 - 1;

  display_fillScreen(DISPLAY_BLACK);
  w = min(display_width(), display_height());
  for (i = 0; i < w; i += 6) {
    i2 = i / 2;
    display_drawRoundRect(cx - i2, cy - i2, i, i, i / 8,
                          display_color565(i, 0, 0));
  }
  return 0;
}

unsigned long display_testFilledRoundRects() {
  int i, i2, cx = display_width() / 2 - 1, cy = display_height() / 2 - 1;

  display_fillScreen(DISPLAY_BLACK);
  for (i = min(display_width(), display_height()); i > 20; i -= 6) {
    i2 = i / 2;
    display_fillRoundRect(cx - i2, cy - i2, i, i, i / 8,
                          display_color565(0, i, 0));
  }
  return 0;
}

#define FILL_SCREEN_DELAY_IN_SEC 3
#define US_PER_SECOND 1000000L
#define MS_PER_SECOND 1000L
unsigned long display_testFillScreen() {
  display_fillScreen(DISPLAY_BLACK);
  printf("BLACK\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  ...
  printf("GREEN\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_DARK_GREEN);
  printf("DARK_GREEN\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  ...
  printf("DARK_BLUE\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_YELLOW);
  printf("YELLOW\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_DARK_YELLOW);
  printf("DARK_YELLOW\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_CYAN);
  printf("CYAN\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_DARK_CYAN);
  printf("DARK_CYAN\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_MAGENTA);
  printf("MAGENTA\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_DARK_MAGENTA);
  printf("DARK_MAGENTA\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_LIGHT_GRAY);
  printf("LIGHT_GRAY\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_GRAY);
  printf("GRAY\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_DARK_GRAY);
  printf("DARK_GRAY\n");
  utils_msDelayDisplayInternal(FILL_SCREEN_DELAY_IN_SEC * MS_PER_SECOND);
  display_fillScreen(DISPLAY_BLACK);
  printf("BLACK\n");

...
// This will init the GPIO hardware so you can write to the 4 LEDs  (LED3 -
// LED0) on the ZYBO board.
int leds_init(bool printFailedStatusFlag) {
  ...
  XGpio_SetDataDirection(&Gpio_LEDs, GPIO_CHANNEL_NUMBER,
                         GPIO_DIRECTION_IS_OUTPUT);
  initLedFlag = 1;
  return STATUS_OK;
}
...
void leds_write(int ledValue) {
  if (initLedFlag) {
    ...
    printf("Error: you must invoke initializeLeds() prior to calling "
           "writeLeds()\n.");
  }
}

// Access LD4 via MIO.
void leds_writeLd4(int ledValue) { mio_writePin(MIO_LD4_MIO_PIN, ledValue); }

// This blinks all of the LEDs for several seconds to provide a visual test of
// the code. This will use a simple for-loop as a delay to keep the code
// independent of other code. Always returns 0 because this is strictly a visual
// test.

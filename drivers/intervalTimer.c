...
// Number used to shift counter 1 when concatenating the 2 counters to get a 64 bit counter
#define INTERVAL_TIMER_SHIFT_VAL       32

// Creates an array of the base address for each timer so that each function can be universal for each timer.
const uint32_t baseAddresses[] = {XPAR_AXI_TIMER_0_BASEADDR, XPAR_AXI_TIMER_2_BASEADDR, XPAR_AXI_TIMER_2_BASEADDR};

// Creates an array of the frequencies for each timer so that each function can be universal for each timer
// Even though though they all have the same frequency currently, this is done so if that ever changes, no updates to 
// the function ever have to be made.
const uint32_t frequencies[] = {XPAR_AXI_TIMER_0_CLOCK_FREQ_HZ, XPAR_AXI_TIMER_1_CLOCK_FREQ_HZ, XPAR_AXI_TIMER_2_CLOCK_FREQ_HZ};

// Reads the timer register based on the timer number provided and using the offset provided
...
{
    uint32_t address = baseAddresses[timerNumber];
    Xil_Out32(address + INTERVAL_TIMER_TCSR0_OFFSET, INTERVAL_TIMER_RESET_VALUE | INTERVAL_TIMER_CASC_BIT_MASK);
...
{
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TCSR0_OFFSET, intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCSR0_OFFSET) | INTERVAL_TIMER_ENT0_BIT_MASK);
}
...
void intervalTimer_reset(uint32_t timerNumber)
{
    ...
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TLR0_OFFSET, INTERVAL_TIMER_RESET_VALUE);
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TCSR0_OFFSET, intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCSR0_OFFSET) | INTERVAL_TIMER_LOAD0_BIT_MASK);
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TCSR0_OFFSET, intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCSR0_OFFSET) & ~INTERVAL_TIMER_LOAD0_BIT_MASK);
    // Reset Counter 1 in the timer
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TLR1_OFFSET, INTERVAL_TIMER_RESET_VALUE);
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TCSR1_OFFSET, intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCSR1_OFFSET) | INTERVAL_TIMER_LOAD1_BIT_MASK);
    Xil_Out32(baseAddresses[timerNumber] + INTERVAL_TIMER_TCSR1_OFFSET, intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCSR1_OFFSET) & ~INTERVAL_TIMER_LOAD1_BIT_MASK);
}
...
{
    // The following creates a variable to hold the concatenation of both counters so that a longer amount of time can be counted
    uint64_t timerTotal = ((uint64_t)intervalTimer_readRegister(timerNumber, INTERVAL_TIMER_TCR1_OFFSET) << INTERVAL_TIMER_SHIFT_VAL) 
